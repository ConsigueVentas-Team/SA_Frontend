...
export default function UserList() {
    ...
    const columns = [
        ...
        },
        {
            header: 'Rol',
            accessorKey: 'rol',
            cell: ({ row }) => <span>{row.original.roles[0].name}</span>,
        },
        {
            header: 'Departamento',
            accessorKey: 'core',
...

        {
            ...
            cell: ({ row }) => (
                <Link
                    to={`/evaluacion/${row.original.id}/${row.original.name}-${row.original.surname}`}
                    key={row.original.id}>
                    <button
    ...
    return (
        <div className='p-2 max-w-5xl- mx-auto'>
            ...
            {isLoading ? (
                <div className='text-gray-400 font-md py-2 px-2'>
                    Cargando ...
                </div>
            ) : (
