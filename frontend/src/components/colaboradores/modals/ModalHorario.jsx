    ...
    const handleAddSchedule = () => {
        if (selectedOptionDay && selectedOption && hours && minutes) {
            ...
            const inicioEntry = schedule.find(
                entry =>
                    entry.option === 'Inicio' && entry.day === selectedOptionDay
            )

            const finEntry = schedule.find(
                entry =>
                    entry.option === 'Fin' && entry.day === selectedOptionDay
            )
    ...
    const enviarDatosAlBackend = scheduleData => {
        ...
        for (const day of daysOfWeek) {
            ...
            if (inicioEntries.length !== 1) {
                errors.push(
                    `Debes agregar una entrada 'Inicio' para el día ${day}.`
                )
            }

            if (finEntries.length !== 1) {
                errors.push(
                    `Debes agregar una entrada 'Fin' para el día ${day}.`
                )
            }

            if (inicioEntries.length === 1 && finEntries.length === 1) {
                const startTime = new Date(
                    `2023-10-26 ${inicioEntries[0].time}`
                )
            ...
            {ayuda !== 'Cargado' ? (
                ...
                <div className='relative w-full max-w-2xl max-h-full text-black bg-white rounded-lg p-5'>
                            ...
                            <div className='inline-flex items-center'>
                                <label
                                    ...
                                    <input
                                        ...
                                        onClick={() =>
                                            setIncluyeDomingo(!incluyeDomingo)
                                        }
                                    />
                            ...
                            {alert && (
                                <div
                                    ...
                                    <span className='block sm:inline'>
                                        El horario se guardo exitosamente.
                                    </span>
                    ...
                    <div className='flex justify-around items-center'>
                        ...
                        <button
                            onClick={onclose}
                            className=' w-1/3 border-2 p-1 mt-3 bg-white font-medium text-cv-primary border-cv-primary hover:text-white hover:bg-cv-primary rounded-lg'>
                            CANCELAR
                        </button>
                    </div>
                </div>
